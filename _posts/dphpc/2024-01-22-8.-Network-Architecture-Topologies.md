...
> Bandwidth B of the cut: Sum of the bandwidths of the cut edges.
> $$B(N_1,N_2)=\sum_{c\in C(N_1,N_2)}b_c$$
> {: .prompt-info}

### Bisection of a Network

In networks often $|V1|$ and $|V2|$ considers terminals separately.
...
- $P = {c_1, c_2, ... c_n}$ where $head(c_i)= tail(c_i+1)$
- Source of P: $tail(c_0)$
- Destination of 𝑃: $head(c_n)$
- Length or hops in a path $𝑃$: $|P|$
...
- Let’s assume $𝑀 = 𝑁 = 2𝑘$, $𝑃 = 𝑀 ⋅ 𝑁: \sqrt 𝑃$
...
$2𝑘^𝑛$ terminals, crossbars of radix $2𝑘$,
$𝑛 𝑘^{𝑛−1}$ of them
...
- 4 $N\times N$ crossbars can be used to build a $2N\times 2N$ crossbar! [Crossbar](/assets/img/ScreenShot%202024-01-08%20at%2012.01.35.png)
