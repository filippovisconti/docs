...
Different entities differ in the number of vantage points they can directly run measurements from. ![shutup](/assets/img/Pasted%20image%2020240116115832.png) Different entities differ in the number of
layers they can directly measure/observe.![shutup](/assets/img/Pasted%20image%2020240116115849.png)
...
Knowing the Internet topology has plenty of useful applications: ![shutup](/assets/img/Pasted%20image%2020240116120752.png)
...
- "non-lossy" links in green ![shutup](/assets/img/Pasted%20image%2020240116121029.png) As user traffic crosses these links, their performance degrades. Upon experiencing performance issues, can users
  precisely locate which links are the culprit?
...
You are looking at a stream of data. There are many questions you might ask: In networking, we usually talk about streams of packets, but these questions apply to other domains as well, e.g. search
...
![shutup](/assets/img/Pasted%20image%2020240116121920.png) ![shutup](/assets/img/Pasted%20image%2020240116121931.png)
...
Idea: use multiple hashes at the same time. An element is considered in the set if all the hash values map to a cell with 1 An element is not in the set if at least one hash value maps to a cell with
0

![shutup](/assets/img/Pasted%20image%2020240116122143.png)

![shutup](/assets/img/Pasted%20image%2020240116122150.png)
...
![shutup](/assets/img/Pasted%20image%2020240116122231.png)

N elements M cells K hash functions FP false positive rate

![shutup](/assets/img/Pasted%20image%2020240116122250.png)

![shutup](/assets/img/Pasted%20image%2020240116122257.png)
...
Because deletions are not possible, the controller may need to regularly reset the bloom filters. Resetting a bloom filter takes some time during which it is not usable. Common trick: use two bloom
filters and use one when the controller resets the other one.

Bloom filters may be extended to allow deletions and to list the filter content.

### CountMin Sketch

How frequently does an element appear? We are going to look at frequencies, i.e. how often an element occurs in a data stream. ![shutup](/assets/img/Pasted%20image%2020240116122719.png)

In the worst case, an algorithm providing exact frequencies requires linear space.

![shutup](/assets/img/Pasted%20image%2020240116122740.png)

Probabilistic data-structures can help again! ![shutup](/assets/img/Pasted%20image%2020240116122758.png)

A CountMin sketch is designed to have provable L1 error bounds for frequency queries. ![shutup](/assets/img/Pasted%20image%2020240116122821.png)
...
A CountMin Sketch uses multiple arrays and hashes. ![shutup](/assets/img/Pasted%20image%2020240116123009.png) To count, increment all hash-indexed fields by 1.
![shutup](/assets/img/Pasted%20image%2020240116123023.png) Hash collisions cause over-counting. Returning the minimum value minimises the error. ![shutup](/assets/img/Pasted%20image%2020240116123043.png)

#### A CountMin sketch recipe

![shutup](/assets/img/Pasted%20image%2020240116123104.png)

#### Limitations

Sketches have limitations. They compute statistical summaries and favour elements with high frequency. ![shutup](/assets/img/Pasted%20image%2020240116123124.png)

![shutup](/assets/img/Pasted%20image%2020240116123130.png)
